...
print("Task B (weakly supervised framework)")

database_manager=Database_manager.make_database_manager()
feature_manager=Features_manager.make_feature_manager()

target="Donald Trump"
target_train="Hillary Clinton"

tweets_train=database_manager.return_train_tweets(filter=target_train)
...
file = open("Task B.csv","w")

#feature_names=numpy.array(['target_in_tweet_labeled','sentiment_labeled'])  #0.702954357569
...
for L in range(1, len(stuff)+1):
    for subset in combinations(stuff, L):
        ...
        X_train,X_test,feature_name=feature_manager.create_feature_space(tweets_test,
                                                                         ...
                                                                         tweets_train,
                                                                         cfg_feature.target_alias['alias_target_DonaldTrump'],
                                                                         cfg_feature.target_alias['alias_pronoums_DonaldTrump'],
                                                                         cfg_feature.target_alias['alias_party_DonaldTrump'],
                                                                         cfg_feature.target_alias['alias_opponents_in_the_party_DonaldTrump'],
                                                                         cfg_feature.target_alias['alias_opponents_out_the_party_DonaldTrump'],
